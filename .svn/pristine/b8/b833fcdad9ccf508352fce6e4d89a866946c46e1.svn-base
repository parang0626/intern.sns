...
// 드롭다운 버튼 클릭
$(document).on('click','.serachTypeDrop',function(){
	var tempsearchType = $(this).text();
	if(tempsearchType == '작성자') {
		searchType = 'name';
		$( '#inputKeyword' ).datepicker("destroy");
		
	} else {
		searchType ='date';
		 $( '#inputKeyword' ).datepicker({
			 dateFormat: "yymmdd"
		    });
		 tempsearchType == '날짜'
	}
	$('#dropSearch').html(tempsearchType+' <span class=\"caret\"></span>');
	...
	$.ajax({
		...
		success : function(data){
			if(data.code == 1) {
				...
				console.log(data.data.userNum+' zz '+$('#detailProfileImage').attr('src'));
...
$(document).on('scroll',document,function(){
	if(loadContentSw){
		...
		var url ='/boards?pageNum=5&lastBoardNum='+$('.boardContent').last().data('boardNum')+'&searchType='+searchType+'&keyWord='+keyword;
...
function getFirstContents() {
	$('#serchConent').html('');
	getBoardAjax('/boards?pageNum=&lastBoardNum=&searchType='+searchType+'&keyWord='+keyword);
}
